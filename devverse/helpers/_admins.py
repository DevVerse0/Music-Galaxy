...
def can_manage_vc(func):
    ...
    async def wrapper(_, update: types.Message | types.CallbackQuery, *args, **kwargs):
        ...
        # Check if user is manually restricted
        if await db.is_restricted(user_id):
            if isinstance(update, types.Message):
                return await update.reply_text("🚫 You are restricted from using bot controls.")
            else:
                return await update.answer("🚫 You are restricted from using bot controls.", show_alert=True)

        # Skip admin check and allow everyone to manage VC
        return await func(_, update, *args, **kwargs)
